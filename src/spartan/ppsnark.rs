...
use crate::{
  ...
  spartan::{
    ...
    sumcheck::{
      engine::{
        InnerSumcheckInstance, MemorySumcheckInstanceV2, OuterSumcheckInstance, SumcheckEngine,
  ...
  <E::Scalar as PrimeField>::Repr: Abomonation,
{
    ...
    T4: SumcheckEngine<E>,
  {
    ...
    let mem_claims = vec![mem_row.final_claims(), mem_row.final_claims()].concat();
  ...
  fn prove(
    ...
    let ((mut outer_sc_inst, mut inner_sc_inst), mem_res) = rayon::join(
      ...
      || {
        ...
        let const_one_vec = vec![E::Scalar::from(1 as u64); mem_row.len()];
        let (mem_res_row, mem_res_col) = rayon::join(
          || {
            MemorySumcheckInstanceV2::<E>::compute_oracles(
              ck,
              &r,
              &gamma,
              vec![
